...
checkpoint = torch.load(f='/scratch/58000.pt')
...
def run_inference(device='cuda', orientation: SliceOrientation = SliceOrientation.SAGITTAL):
    ...
    per_point_errs = []
    subject_large_structure_dice = []
    subject_small_structure_dice = []

    large_structure_names = ['CTX', 'CP', 'HPF', 'BS', 'CB', 'CBX']
    small_structure_names = ['act', 'fr', 'mtt', 'IPN', 'MH', 'LH']

    for subject in tqdm(val_subjects_sample, desc=f'running inference'):
        logger.info(f'Run inference for {subject}')

        subject_val_samples = val_samples[val_samples[:, 0] == subject]

        ccf_annotations_large_structure_dice_metric = SparseDiceMetric(
            class_ids=[688, 672, 1089, 343, 512, 528],
            terminology_path=Path('/root/capsule/data/ccf_terminology/terminology.csv'),
            terminology_correction_path=Path('/root/capsule/scratch/missing_ccf_ids_mapped.json')
        )
        ccf_annotations_small_structure_dice_metric = SparseDiceMetric(
            class_ids=[908, 595, 690, 100, 483, 186],
            terminology_path=Path('/root/capsule/data/ccf_terminology/terminology.csv'),
            terminology_correction_path=Path('/root/capsule/scratch/missing_ccf_ids_mapped.json')
        )
        ...
        val_dataloader = create_dataloader(
            ...
            tissue_bboxes_path=Path(
                '/root/capsule/data/tissue_bounding_boxes/tissue_bounding_boxes.parquet'),
            batch_size=16,
            num_workers=4,
        ...
        for batch in tqdm(val_dataloader):
            ...
            pred_tissue_mask_logits = model_out[:, -1]
            ...
            pred_tissue_masks = F.sigmoid(pred_tissue_mask_logits) > 0.5
            ...
            batch_point_idx = batch_coords_index[:, template_slice_idx][
                tissue_masks.bool()].flatten()
            ...
            for sample_idx in range(n):
                for dice_metric in (ccf_annotations_large_structure_dice_metric,
                                    ccf_annotations_small_structure_dice_metric):
                    _update_dice_metric(
        ...
        per_slice_median_err_mins.append(
            subject_slice_median_errs[~torch.isnan(subject_slice_median_errs)].min().item())
        per_slice_median_err_maxs.append(
            subject_slice_median_errs[~torch.isnan(subject_slice_median_errs)].max().item())
        ...
        subject_large_structure_dice.append(
            ccf_annotations_large_structure_dice_metric.compute(reduce=None))
        subject_small_structure_dice.append(
            ccf_annotations_small_structure_dice_metric.compute(reduce=None))
    ...
    subject_large_structure_dice = np.stack(subject_large_structure_dice)
    subject_small_structure_dice = np.stack(subject_small_structure_dice)
    avg_dice_large_structure = np.mean(subject_large_structure_dice, axis=1)
    avg_dice_small_structure = np.mean(subject_small_structure_dice, axis=1)
    ...
    fig, ax = plt.subplots(figsize=(10, 5))
    sns.scatterplot(
        x=torch.cat(per_point_template_indexs),
        y=torch.cat(per_point_errs),
        ax=ax,
        s=1,
        alpha=0.3
    )
    ...
    sns.boxplot(data=subject_large_structure_dice, ax=ax[0, 0])
    ax[0, 0].set_xticklabels(large_structure_names)
    ax[0, 0].set_ylabel('Dice score')
    ax[0, 0].set_title('Large structure region-wise dice score')

    sns.boxplot(y=avg_dice_large_structure, ax=ax[0, 1])
    ax[0, 1].set_ylabel('Dice score')
    ax[0, 1].set_title('Large structure average dice score')

    sns.boxplot(data=subject_small_structure_dice, ax=ax[1, 0])
    ax[1, 0].set_xticklabels(small_structure_names)
    ax[1, 0].set_ylabel('Dice score')
    ax[1, 0].set_title('Small structure region-wise dice score')

    sns.boxplot(y=avg_dice_small_structure, ax=ax[1, 1])
    ax[1, 1].set_ylabel('Dice score')
    ax[1, 1].set_title('Small structure average dice score')

    fig.savefig('/scratch/dice.png')


if __name__ == '__main__':
    tissue_bboxes = pd.read_parquet(
        '/root/capsule/data/tissue_bounding_boxes/tissue_bounding_boxes.parquet').set_index(
        'subject_id')
